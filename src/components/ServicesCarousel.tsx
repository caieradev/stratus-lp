...
export default function ServicesCarousel({className = ""}) {
  const settings = {
    dots: false,
    infinite: true,
    speed: 500,
          ...
          <div key={index} className="w-full flex justify-center items-center relative overflow-visible">
            ...
            >
              <Image src={image.url} alt={`Slide ${index + 1}`}
                ...
                height={400}
              />
              <div className='bg-primary rounded-b-3xl absolute bottom-0 left-1/2 transform -translate-x-1/2 w-5/6'>
                <p className='w-full text-center text-white text-4xl py-6'>{image.caption}</p>
              </div>
            </div>
          </div>
        ))}
